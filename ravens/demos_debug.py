...
# ======================================================================================================================
# Generate training and testing data (saved locally). Note: remove --disp for headless mode.
# headless mode: python ravens/demos.py --assets_root=./ravens/environments/assets/ --disp=True --task=block-insertion --mode=train --n=10
# we are using: python ravens/demos.py --assets_root=./ravens/environments/assets/ --task=block-insertion --mode=train --n=10
FLAGS.assets_root = '../ravens/environments/assets/'
FLAGS.disp = True
FLAGS.task = 'block-insertion'
FLAGS.mode = 'train'  # 'test'
FLAGS.n = 50
...
def main(unused_argv):
    ...
    env_cls = ContinuousEnvironment if FLAGS.continuous else Environment  # ravens/environments/environment.py/class Environment(gym.Env):
    env = env_cls(
        FLAGS.assets_root,
        disp=FLAGS.disp,
        shared_memory=FLAGS.shared_memory,
        hz=480)
    task = tasks.names[FLAGS.task](continuous=FLAGS.continuous)  # task实例化: 比如BlockInsertion
    task.mode = FLAGS.mode  # 'train'
